...
function showTranslation() {
  ...
  $('translate-hint').textContent = target === 'off' ? 'Recognition only.'
    ...
    : `Hy-MT2 1.8B on ${mtDevice}. Per sentence, grey preview.`;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.onmessage = event => {
      ...
      if (data.type === 'ready') {
        ...
        $('translation-state').textContent = data.translate ? `Hy-MT2 · 1.8B · ${mtDevice}` : current.translate_error ? 'Translation unavailable' : 'Not translating';
        resolve();
      }
...
function begin(source, processing) {
  ...
  $('translation-state').textContent = translating() ? `Hy-MT2 · 1.8B · ${mtDevice}` : 'Not translating'; $('timer').textContent = '00:00';
  $('session-engine').textContent = modelNames[selected];
...
$('export').addEventListener('click', () => {
  ...
  const blob = new Blob([JSON.stringify({app: 'R2D2', version: appVersion, policy: '160ms hop / 160ms lookahead / 8s window / 1 token rollback / merges up to 3 hops when behind',
    translation_policy: `Hy-MT2-1.8B Q4_K_M on ${mtDevice} / greedy / settle per closed sentence / latest-wins draft`, runs: entries}, null, 2)], {type: 'application/json'});
